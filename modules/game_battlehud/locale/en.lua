-- locale/en.lua
--
-- Every UI string the HUD displays. No English string is hardcoded inside
-- a widget; widgets call Locale.t(key, args).
--
-- Keys are namespaced. `battle.event.*` keys correspond to event kinds
-- from §3.3 so the text log can localize raw resolve events uniformly.
...
return {
  ...
  -- Event log (one key per recognised §3.3 event kind)
